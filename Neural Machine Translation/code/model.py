...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        hs = self.rnn(embed)[0]  # TO check si, il y a deux elements dans cette list
        return hs
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        output_concatenad = torch.cat([source_context, output], dim=2)

        output_concatenad = self.ff_concat(output_concatenad)

        output_concatenad = torch.tanh(output_concatenad)
        output_concatenad = self.predict(output_concatenad)

        #prediction = torch.softmax(output_concatenad,dim = 2)
        prediction = output_concatenad
        return prediction, h
